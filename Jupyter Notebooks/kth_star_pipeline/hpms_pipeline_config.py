...
class PipelineConfig:
    def __init__(self, catalog=None, query_string=None,
                 xmatch_max_neighbors=None, max_neighbor_dist=None, min_neighbors=None,
                 k=None, max_obj_deviation=None, id_col_name=None, 
                 mag_cols=None, mag_err_cols=None, debug_mode=False):

        # Catalog Specific Parameters:
        self.catalog = catalog
        self.id_col_name = id_col_name
        self.mag_cols = mag_cols
        self.mag_err_cols = mag_err_cols

        # Filtering Specific Parameters:
        self.query_string = query_string
        self.xmatch_max_neighbors = xmatch_max_neighbors
        self.max_neighbor_dist = max_neighbor_dist
        self.min_neighbors = min_neighbors
        self.k = k
        self.max_obj_deviation = max_obj_deviation
        
        # Additional Pipeline Parameters:
        self.debug_mode = debug_mode
            

def PipelineConfig_from_dict(d):
...
def kth_star_min_distance(group, k, mag_cols, mag_err_cols, max_obj_deviation, debug_mode=False):
    mag_cols_2 = [f"{col}_2" for col in mag_cols]
    mag_err_cols_2 = [f"{col}_2" for col in mag_err_cols]
    neighbors = [
        Star(row.RA_2, row.DEC_2, 
             [getattr(row, col) for col in mag_cols_2], 
             [getattr(row, col) for col in mag_err_cols_2])
...
def apply_kth_star(df, k, id_col, mag_cols, mag_err_cols, max_obj_deviation, debug_mode=False):
    ...
    if df.empty:
        ...
        hpms_cols = (
            df.groupby(id_col)
              .apply(kth_star_min_distance, k=k-1, mag_cols=mag_cols, mag_err_cols=mag_err_cols,
                     max_obj_deviation=max_obj_deviation, debug_mode=debug_mode)
...
def execute_pipeline(config):
    ...
    star_filtered = config.catalog.query(config.query_string)

    # Crossmatch catalog with itself and filter for group sizes >= min_ neighbors
    xmatch = star_filtered.crossmatch(
                star_filtered,
                n_neighbors=config.xmatch_max_neighbors,
                radius_arcsec=config.max_neighbor_dist,
                suffixes=['_1', '_2']
    )
    # if debug_mode: 
    #     print(f"Length of self crossmatch: {len(xmatch.compute())}")
        
    neighbors_filtered = xmatch.map_partitions(n_neighbors_filter, config.min_neighbors, config.id_col_name)

    if config.debug_mode: print(f"Length after neighbors filter: {len(neighbors_filtered.compute())}")
    
    # Add column for kth minimum distance and postfiltering parameters
    kth_star = neighbors_filtered.map_partitions(apply_kth_star, config.k, config.id_col_name, 
                                                 config.mag_cols, config.mag_err_cols, config.max_obj_deviation, 
                                                 config.debug_mode)
    
    if config.debug_mode: print(f"Length after kth star filter: {len(kth_star.compute())}")
    kth_star_filtered = kth_star.query(f'kth_min_deviation < {config.max_obj_deviation}')
...
def merge_groups_and_reduce(df, id_1, max_obj_deviation):
    ...
    for group_id, group_df in df.groupby(id_1):
        ...
        stars_2 = filtered_group_df[id_2].unique()
        star_1 = filtered_group_df[id_1][0]
        # Connect all pairs of stars in this group
        for i in range(len(stars)):
            G.add_edge(star_1, stars_2[i])
